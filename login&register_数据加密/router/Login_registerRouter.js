...
router.post('/login',(request,response)=>{
    ...
    usersModel.findOne({email,password:SHA1(MD5(password))},function (err,data){
        if(data){
            //1.为请求开辟一个session会话存储空间
            //2.将客户端与服务器沟通产生的数据放入session会话存储空间
            //3.获取session会话存储空间的id
            //4.返回给客户端一个cookie，包含着session会话存储空间的id
            //如果使用了一些第三方库，以上四步，一行代码搞定

            //response.cookie('_id',data._id,{maxAge:1000*30})
            request.session._id = data._id.toString()
            response.redirect('http://localhost:3000/user_center')
        }else if(err){
...
router.post('/register',(request,response)=>{
    ...
    usersModel.findOne({email},function (err,data){
        ...
        usersModel.create({email,name,password:SHA1(MD5(password))},function (err,data){
            if(!err){
